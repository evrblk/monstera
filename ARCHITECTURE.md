...
- `monstera.yaml`: `go_code.{output_package,core_types_package}`; `cores[].{read,update}_methods[].{name,
  ...
  (`<Stub>MonsteraStub` + `<Stub>NonclusteredStub`), `validation.go` (`<Core>ValidatingCore` — see below).
- Per method `Foo`: needs `FooRequest`/`FooResponse` proto in `core_types_package`, each implementing
  `encoding.BinaryMarshaler`/`Unmarshaler`; every `*Request` implements `Validate() error`; sharded
  `*Request` also implements `ShardKey() cluster.ShardKey`. These (MarshalBinary + Validate + ShardKey)
  are **user-provided**, NOT generated by monstera — codegen's `request`/`unshardedRequest` interface
  constraints just make the package fail to compile without them. Method numbers are wire-stable; never
  renumber.
- Request validation: every generated `New<Core>CoreAdapter(..., core)` wraps `core` in a generated
  `New<Core>ValidatingCore(core)` before storing it — unconditional, inside the constructor, not at each
  call site, so nothing can construct the adapter around a core that skips validation. `*ValidatingCore`
  implements the same `*CoreApi`; each method calls `Validate()` on the request payload first and, on
  error, returns `ApplicationError: mrpc.NewError(mrpc.InvalidRequest, err.Error())` without calling the
  wrapped core. `<Stub>MonsteraStub` also calls `Validate()` client-side before marshaling — a separate,
  best-effort optimization (skip a wasted round trip), not the safety guarantee. Wrap a bare core in its
  `*ValidatingCore` in core-level unit tests too, so tests exercise `Validate()` like production traffic
  does without changing how the tests call the core.
