...
func generateNonclusteredStub(f *File, stub *MonsteraStub, cores []*MonsteraCore, cfg *MonsteraYaml, emittedAdapters map[string]bool) {
	...
	for _, core := range cores {
		...
		f.Type().Id(adapterTypeName(core.Name)).StructFunc(func(g *Group) {
			...
			g.Id("id").String()
			g.Id("lowerBound").Qual(clusterPkg, "ShardKey")
			g.Id("upperBound").Qual(clusterPkg, "ShardKey")
		})
		f.Line()
	}
	...
	f.Type().Id(applicationCoresFactoryType).StructFunc(func(g *Group) {
		for _, core := range cores {
			g.Id(core.Name+"CoreFactoryFunc").Func().Params(Id("shardId").String(), Id("lowerBound").Qual(clusterPkg, "ShardKey"), Id("upperBound").Qual(clusterPkg, "ShardKey")).Qual(cfg.GoCode.OutputPackage, core.Name+"CoreApi")
		}
	})
	...
	f.Func().Id("New"+stubType).Params(
		...
		g.For(Id("i").Op(":=").Lit(0), Id("i").Op("<").Id("shardsPerApp"), Id("i").Op("++")).BlockFunc(func(g *Group) {
			g.Id("lowerBound").Op(":=").Qual(clusterPkg, "ShardKey").Call(Int64().Call(Id("i")).Op("*").Id("shardSize"))
			g.Id("upperBound").Op(":=").Qual(clusterPkg, "ShardKey").Call(Int64().Call(Id("i").Op("+").Lit(1)).Op("*").Id("shardSize").Op("-").Lit(1))
			g.Line()

			g.List(Id("sl"), Id("su")).Op(":=").Qual(clusterPkg, "ShortenBounds").Call(Id("lowerBound").Dot("Bytes").Call(), Id("upperBound").Dot("Bytes").Call())
			g.Line()
