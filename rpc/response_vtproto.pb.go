...
func (m *Request) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.IdempotencyToken) > 0 {
		i -= len(m.IdempotencyToken)
		copy(dAtA[i:], m.IdempotencyToken)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.IdempotencyToken)))
		i--
		dAtA[i] = 0x22
	}
	if m.Now != 0 {
	...
	if m.MethodNumber != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.MethodNumber))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
...
func (m *Request) SizeVT() (n int) {
	...
	_ = l
	if m.MethodNumber != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.MethodNumber))
	}
	l = len(m.Data)
	if l > 0 {
		n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
...
func (m *Error) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			...
			m.Context = append(m.Context, &ErrorContext{})
...
func (m *Request) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field MethodNumber", wireType)
			}
			m.MethodNumber = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.MethodNumber |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field IdempotencyToken", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.IdempotencyToken = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
